...
def gauss_noise(_spec, stdev):
    ...
    awgn = torch.normal(0, stdev, (f,t)).to(_spec.device)
...
class GaussNoise(torch.nn.Module):
    ...
    def __init__(self, stdev_gen=1e-1):
        super().__init__()
...
class SpecAugment(torch.nn.Module):
    def __init__(self, time_mask_param=0, freq_mask_param=0, num_time_masks=1, num_freq_masks=1):
        super().__init__()
        self.time_mask_param = time_mask_param
        self.freq_mask_param = freq_mask_param
        self.num_time_masks = num_time_masks
        self.num_freq_masks = num_freq_masks
        self.time_mask = T.TimeMasking(time_mask_param) if time_mask_param > 0 else None
        self.freq_mask = T.FrequencyMasking(freq_mask_param) if freq_mask_param > 0 else None
